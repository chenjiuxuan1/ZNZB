...
import { analyzeMetabaseAnomalyBatch } from "./metabase-anomaly-agent.mjs";
...
const FILES = {
  ...
  anomalyAnalyses: "config/metabase-anomaly-analyses.json",
};
...
export function createPlatformApi({
  ...
  return {
    ...
    async prepareMetabaseInvestigationBatches(cases) {
      return buildDashboardAnalysisJobs(cases);
    },

    async completeMetabaseAnomalyBatch(batchId, results) {
      return completeMetabaseAnomalyBatch({ rootDir, batchId, results });
    },

    async finalizeAiFirstMetabasePatrol(options = {}) {
      return finalizeAiFirstMetabasePatrol({
        rootDir,
        ...options,
      });
    },
