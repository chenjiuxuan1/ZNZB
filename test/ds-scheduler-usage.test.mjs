...
test("tokenUser maps token to DS username", () => {
  assert.equal(tokenUser("REPLACE_WITH_DS_USER_TOKEN_02"), "jiangchuanchen");
  assert.equal(tokenUser("REPLACE_WITH_DS_USER_TOKEN_01"), "yannhao");
...
test("buildCountryUsage exposes user field per token operator", () => {
  const rows = [
    { operation_time: "2026-08-20 09:00:00", operator: "张三", country: "ph", action: "list_projects", success: 1, risk_level: "low", duration_ms: 100, token: "REPLACE_WITH_DS_USER_TOKEN_01" },
  ...
  assert.equal(byUser["REPLACE_WITH_DS_USER_TOKEN_01"], "yannhao");
...
test("tokenUser prefers fetched tokenUserMap over static map", () => {
  const map = { "REPLACE_WITH_DS_USER_TOKEN_01": "yannhao-renamed" };
  assert.equal(tokenUser("REPLACE_WITH_DS_USER_TOKEN_01", map), "yannhao-renamed");
  assert.equal(tokenUser("REPLACE_WITH_DS_USER_TOKEN_02", {}), "jiangchuanchen");
