...
test("acceleration is disabled by default and clamps safe resource limits", () => {
  assert.deepEqual(getMetabaseAnomalyAccelerationSettings({}), { enabled: false, maxConcurrency: 2, snapshotTtlSeconds: 600 });
  ...
  }), { enabled: true, maxConcurrency: 3, snapshotTtlSeconds: 60 });
});
