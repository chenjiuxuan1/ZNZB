...
let model = { result: null, loading: false, error: "", country: "", status: "", keyword: "", date: "" };
...
export function renderDsFailureLogs(root) {
  ...
  root.innerHTML = `<section class="panel"><p class="muted">正在读取六个国家的 DS 失败任务与自动修复状态…</p></section>`;
  void load(root);
}

async function load(root, { force = false } = {}) {
  if (model.loading && !force) return;
  model.loading = true;
  model.error = "";
  paintIfReady(root);
  try {
    const query = model.date ? `?date=${encodeURIComponent(model.date)}` : "";
    model.result = await apiGet(`/api/ds-failure-logs${query}`);
  } catch (error) {
...
function paintIfReady(root) {
  if (model.result) paint(root);
}
...
function paint(root) {
  ...
  root.innerHTML = `
    <div class="page-header batch-hero ds-failure-hero">
      <div>
        ...
        <p class="page-note">读取六个国家已配置项目的失败实例，判断失败后是否出现成功实例，并展示底层失败任务日志原因。</p>
      </div>
      <div class="hero-stats">
        ${stat("已接入国家", `${result.configuredCountries || 0} / 6`)}
        ${stat("失败任务", result.totalFailures ?? "—")}
        ${stat("已自动修复", result.recoveredCount ?? "—")}
        ${stat("修复中 / 待修复", result.totalFailures != null ? `${result.repairingCount || 0} / ${result.unresolvedCount || 0}` : "—")}
      </div>
    </div>
    ...
    <section class="panel ds-failure-toolbar">
      <div class="detail-header compact-header">
        <div><h2 class="panel-title">失败任务筛选</h2><p class="muted">默认查看各国当地今天的数据；“已自动修复”表示失败实例之后已经出现成功实例。</p></div>
        <button class="primary" id="ds-failure-refresh" ${model.loading ? "disabled" : ""}>${model.loading ? "正在刷新…" : "刷新失败日志"}</button>
      </div>
      <div class="ds-failure-filter-grid">
        <label>日期<input id="ds-failure-date" type="date" value="${escapeHtml(model.date)}"></label>
  ...
  root.querySelector("#ds-failure-refresh")?.addEventListener("click", () => load(root, { force: true }));
  root.querySelector("#ds-failure-date")?.addEventListener("change", (event) => { model.date = event.target.value; void load(root, { force: true }); });
